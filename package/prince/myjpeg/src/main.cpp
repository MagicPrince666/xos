...
int main(int argc, char *argv[])
{
       ...
       fb_stat(fbdev, (int *)&fb_width, (int *)&fb_height, (int *)&fb_depth);
       ...
       if ((cinfo.output_width > fb_width) ||
              ...
              std::cout <<"Picture width = " << cinfo.output_width << std::endl;
              std::cout <<"Picture height = " << cinfo.output_height << std::endl;
              std::cout <<"Lcd width = " << fb_width << std::endl;
              std::cout <<"Lcd height = " << fb_height << std::endl;
              std::cout << "too large JPEG file,cannot display"<< std::endl;
              //std::cout << str << now << ".jpeg"<< std::endl;
              sprintf(name, "%ld.jpeg", now);
              std::string str(name);
              generate_image_thumbnail( argv[1] , str.c_str() );
              return (-1);
       }
...
{
       caddr_t         fbmem;
       if ((fbmem = (caddr_t)mmap(0, screensize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)) == MAP_FAILED) {
              perror(__func__);
              return (void *) (-1);
       }
